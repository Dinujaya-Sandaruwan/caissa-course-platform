...
export default function EditCoursePage() {
  ...
  // Preview video state (Bunny.net)
  const [bunnyPreviewVideoId, setBunnyPreviewVideoId] = useState("");

  // Fetch course on mount
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const res = await fetch(`/api/coach/courses/${courseId}`);
        ...
        setMetadata({
          ...
          maxDiscountPercent: data.maxDiscountPercent
            ? data.maxDiscountPercent.toString()
            : "",
        ...
        if (data.bunnyPreviewVideoId) {
          setBunnyPreviewVideoId(data.bunnyPreviewVideoId);
          setMetadata((prev) => ({
            ...prev,
            bunnyPreviewVideoUrl: data.bunnyPreviewVideoUrl,
          }));
        }
        ...
        const mappedChapters: ChapterDraft[] = data.chapters.map((ch: any) => ({
          ...
          lessons: ch.lessons.map((l: any) => ({
            ...
            materials: l.materials.map((m: any) => ({
              id: m._id || generateId(),
              file: null,
              fileName: m.title || "Existing Material",
              fileSize: 0,
              title: m.title,
              status: "uploaded",
              progress: 100,
            ...
            signedIframeUrl: l.signedIframeUrl,
            videoStatus:
              l.videoStatus === "ready"
                ? "ready"
                : l.bunnyVideoId
        ...
        setStep2Error("Failed to load course data");
      } finally {
  ...
  const confirmDelete = () => {
    ...
    if (itemToDelete.type === "chapter") {
      if (!itemToDelete.chapterId.startsWith("draft_")) {
        setDeletedChapterIds((prev) => [...prev, itemToDelete.chapterId]);
      }
      setChapters((prev) =>
        prev.filter((ch) => ch.id !== itemToDelete.chapterId),
      );
    } else if (itemToDelete.type === "lesson") {
      if (!itemToDelete.lessonId.startsWith("draft_")) {
        setDeletedLessonIds((prev) => [
          ...prev,
          {
            chapterId: itemToDelete.chapterId,
            lessonId: itemToDelete.lessonId,
          },
        ]);
      }
      setChapters((prev) =>
  ...
  // handleVideoUploadComplete — called by VideoUploader when a lesson video upload finishes
  const handleVideoUploadComplete = (
    chapterId: string,
    lessonId: string,
    videoId: string,
  ) => {
    updateLessonField(chapterId, lessonId, {
      bunnyVideoId: videoId,
      videoStatus: "processing",
    });
  };

  // handlePreviewVideoUploadComplete — called by VideoUploader when preview video upload finishes
  const handlePreviewVideoUploadComplete = (videoId: string) => {
    setBunnyPreviewVideoId(videoId);
  };
  ...
  const handleSubmitCourse = async () => {
    ...
    try {
      ...
      setSubmitStatus("Processing deletions...");
      for (const chId of deletedChapterIds) {
        await fetch(`/api/coach/courses/${courseId}/chapters/${chId}`, {
          method: "DELETE",
        });
      }
      for (const lItem of deletedLessonIds) {
        // Skip learning lessons belonging to deleted chapters because the chapter deletion cascade takes care of it
        if (!deletedChapterIds.includes(lItem.chapterId)) {
          await fetch(
            `/api/coach/courses/${courseId}/chapters/${lItem.chapterId}/lessons/${lItem.lessonId}`,
            {
              method: "DELETE",
            },
          );
      ...
      // 2. Update the course draft
      setSubmitStatus("Saving course details...");
      ...
      // Attach preview video if newly uploaded to Bunny
      if (bunnyPreviewVideoId) {
        coursePayload.bunnyPreviewVideoId = bunnyPreviewVideoId;
      }
      ...
      // 3. Process chapters and lessons sequentially
      for (let chIdx = 0; chIdx < chapters.length; chIdx++) {
        const ch = chapters[chIdx];
        let currentChapterId = ch.id;
        const isNewChapter = currentChapterId.startsWith("draft_");
        ...
        if (isNewChapter) {
          ...
          if (!chapterRes.ok) {
            ...
            throw new Error(
              err.error || `Failed to create chapter: ${ch.title}`,
            );
          }
          const chapterData = await chapterRes.json();
          currentChapterId = chapterData._id;
        } else {
          setSubmitStatus(
            `Updating chapter ${chIdx + 1}/${chapters.length}: ${ch.title}...`,
          );
          const chapterRes = await fetch(
            `/api/coach/courses/${courseId}/chapters/${currentChapterId}`,
            {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ title: ch.title.trim() }),
          ...
          if (!chapterRes.ok) {
            const err = await chapterRes.json();
            throw new Error(
              err.error || `Failed to update chapter: ${ch.title}`,
            );
          }
        }

        // 4. Process lessons for this chapter
        for (let lIdx = 0; lIdx < ch.lessons.length; lIdx++) {
          const lesson = ch.lessons[lIdx];
          const isNewLesson = lesson.id.startsWith("draft_");

          setSubmitStatus(
            `${isNewLesson ? "Creating" : "Updating"} lesson ${lIdx + 1}/${ch.lessons.length} in "${ch.title}"...`,
          );
          ...
          if (lesson.bunnyVideoId) {
            lessonPayload.bunnyVideoId = lesson.bunnyVideoId;
          }

          const tempMats = lesson.materials.filter((m) => m.tempPath);
          if (tempMats.length > 0) {
            lessonPayload.tempMaterials = tempMats.map((m) => ({
          ...
          // Existing materials
          const existingMats = lesson.materials.filter(
            (m) => m.url && !m.tempPath,
          );
          if (!isNewLesson) {
            lessonPayload.existingMaterials = existingMats.map((m) => ({
              title: m.title,
              url: m.url,
            }));
          }
          ...
          const endpoint = isNewLesson
            ? `/api/coach/courses/${courseId}/chapters/${currentChapterId}/lessons`
            : `/api/coach/courses/${courseId}/chapters/${currentChapterId}/lessons/${lesson.id}`;

          const lessonRes = await fetch(endpoint, {
            method,
            headers: { "Content-Type": "application/json" },
          ...
          if (!lessonRes.ok) {
            ...
            throw new Error(
              err.error ||
                `Failed to ${method === "POST" ? "create" : "update"} lesson: ${lesson.title}`,
            );
          }
        }
      }

      setSubmitStatus("Changes saved successfully! Redirecting...");
      setTimeout(() => {
        ...
        <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 relative overflow-hidden">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm font-medium mt-0.5">
              Review your course details before saving your changes
            </p>
          </div>
        </div>
      ...
      <div className="space-y-8 animate-[fade-in-up_0.4s_ease-out]">
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.04)] ring-1 ring-gray-900/5">
          ...
          <VideoUploader
            ...
            signedIframeUrl={metadata.bunnyPreviewVideoUrl}
          />
        </div>
                        ...
                        <div className="flex flex-col gap-3 w-full p-1.5 pt-0">
                          ...
                          <div className="pl-9 pr-2">
                            <VideoUploader
                              ...
                              existingVideoId={lesson.bunnyVideoId}
                              videoStatus={lesson.videoStatus}
                              signedIframeUrl={lesson.signedIframeUrl}
                            />
                          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div>
              ...
              >
                {metadata.thumbnailUploadStatus === "success" &&
                (metadata.thumbnailFile || metadata.tempThumbnailPath) ? (
                    ...
                    <img
                      src={
                        metadata.thumbnailFile
                          ? URL.createObjectURL(metadata.thumbnailFile)
                          : (metadata.tempThumbnailPath || "").replace(
                              /^\/?(\.\/)?public\//,
                              "/",
                            )
                      }
