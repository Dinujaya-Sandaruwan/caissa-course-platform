...
export default function CreateCoursePage() {
  ...
  const handleSubmitCourse = async () => {
    ...
    try {
      ...
      // 1b. Patch preview video if uploaded to Bunny
      if (bunnyPreviewVideoId) {
        setSubmitStatus("Saving preview video...");
        const pvRes = await fetch(`/api/coach/courses/${courseId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ bunnyPreviewVideoId }),
        });
        if (!pvRes.ok) {
          console.error("Failed to save preview video");
        }
      }
  ...
  if (step === 3) {
    ...
    const lessonsWithVideo = chapters.reduce(
      (acc, ch) => acc + ch.lessons.filter((l) => l.bunnyVideoId).length,
      0,
    );
        ...
        <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 relative overflow-hidden">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm font-medium mt-0.5">
              Double-check your course details before submitting for review
            </p>
          </div>
        </div>
    ...
    return (
      <div className="space-y-8 animate-[fade-in-up_0.4s_ease-out]">
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.04)] ring-1 ring-gray-900/5">
          ...
          <VideoUploader
            title={`Preview — ${metadata.title || "Course"}`}
            onUploadComplete={handlePreviewVideoUploadComplete}
            existingVideoId={bunnyPreviewVideoId || null}
          />
        </div>
                        ...
                        <div className="flex flex-col gap-3 w-full p-1.5 pt-0">
                          ...
                          <div className="pl-9 pr-2">
                            <VideoUploader
                              ...
                              existingVideoId={lesson.bunnyVideoId || null}
